...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print("=> creating model '{}'".format(args.arch))  #here
    # model = models.__dict__[args.arch]()
    model = resnet50(num_classes=54)
    print("11111111111")
    num_ftrs = model.fc.in_features
    model.fc = nn.Linear(num_ftrs, args.num_classes)

    if args.distributed:  #? how to
        ...
        if args.arch.startswith('alexnet') or args.arch.startswith('vgg'):
            ...
            model = torch.nn.DataParallel(model).cuda()  #here

    # define loss function (criterion) and optimizer
    criterion = nn.CrossEntropyLoss().cuda(args.gpu)
    # criterion  = focal_loss( num_classes=54)
    ...
    if args.eval_pth != '':
        if os.path.exists(args.eval_pth) and (not os.path.isdir(args.eval_pth)):
            if args.eval_pth.startswith('s3://'):
                model_name = args.eval_pth.rsplit('/', 1)[1]
                shutil.copy(args.eval_pth, '/cache/tmp/' + model_name)
